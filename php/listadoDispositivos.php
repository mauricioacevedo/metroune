<?php
	...
	$sql="SELECT nombre,( select a.nombre from tipos_dispositivos a where a.id=tipo) as disp,vendor,modelo,estado,id,serial FROM dispositivo order by serial ASC";
...
<script language="javascript">
	...
	function consulta(){
		...
		location.href="./listadoDispositivos.php?operacion=consulta&paramSearch="+optionSearch+"&valueSearch="+parametroBusqueda.value;
	}

	function enInvestigacion(serial){
		if(confirm('El dispositivo con serial"+serial+" se pasara a la tabla de Seguimiento e Investigacion.\nDesea continuar?')){
            location.href="./listadoDispositivos.php?operacion=enInvestigacion&serial="+serial;
        }
...
<p>&nbsp;</p>
<p align="center"><strong><font size="+3">Listado de Equipos de Medida en Control Metrológico</font></strong></p>
  	...
  	for($i=0;$i<$rows;$i++){
		...
		$serial = pg_result($result,$i,6);
		  ...
		  echo "<tr bgcolor='".$bg."'>";
			...
			$operacion="$operacion<br><a style=\"color: red;\" href='javascript:enInvestigacion(\"$serial\")'>Investigac&iacute;on</a>";
